...
def get_args():
    ...
    parser.add_argument("--data_hours", type=int, default=24,
        ...
        Type: int. Default: 24.")
    parser.add_argument("--gap_time", type=int, default=12,                         help="The gap between data and when we are making predictions. Type: int. Default: 12.")
    ...
    parser.add_argument("--cohort_filepath", type=str, help="This is the filename containing a numpy         array of length len(X), containing the cohort membership for each example in X. This file should be         saved in the folder 'cluster_membership'. Only applies to cohorts == 'custom'. Type: str.")
    parser.add_argument("--sample_weights", action="store_true", default=False, help="This is an indicator         flag to weight samples during training by their cohort's inverse frequency (i.e. smaller cohorts will be         more highly weighted during training).")
...
def stratified_split(X, Y, cohorts, train_val_random_seed=0):
    ...
    X_train_val, X_test, y_train_val, y_test,         cohorts_train_val, cohorts_test =         train_test_split(X, Y, cohorts, test_size=0.2,
                         random_state=train_val_random_seed, stratify=Y)

    X_train, X_val, y_train, y_val,         cohorts_train, cohorts_val =         train_test_split(X_train_val, y_train_val, cohorts_train_val, test_size=0.125,
...
def run_separate_models(X_train, y_train, cohorts_train,
    ...
    if FLAGS.test_time:
        for task in all_tasks:
            ...
            model_path = FLAGS.experiment_name +                 '/models/' + "_".join(model_fname_parts)
    ...
    for task in all_tasks:
        ...
        model_fname_parts = ['separate', str(task), 'lstm_shared', str(FLAGS.num_lstm_layers), 'layers', str(FLAGS.lstm_layer_size), 'units',
                             str(FLAGS.num_dense_shared_layers), 'dense_shared', str(FLAGS.dense_shared_layer_size), 'dense_units', 'mortality']
        model_dir = FLAGS.experiment_name +             '/checkpoints/' + "_".join(model_fname_parts)
...
#         saps = pd.read_csv('data/saps.csv')
        ...
        cmo = pd.read_csv('code_status.csv')
        cmo = cmo[cmo.cmo > 0]
        #use dncpr_first_charttime instead of timednr_chart
...
#     K.tensorflow_backend.set_session(tf.Session(config=config))
    ...
    fname_keys = FLAGS.experiment_name + '/results/' +         '_'.join([FLAGS.model_type.lower(), 'model_keys', sw]) + '.npy'
    fname_results = FLAGS.experiment_name + '/results/' +         '_'.join([FLAGS.model_type.lower(), 'model_results', sw]) + '.npy'
    ...
    if os.path.exists(fname_keys) and not FLAGS.repeats_allowed:
        ...
        if FLAGS.model_type == "MULTITASK":
            current_run = current_run +                 [FLAGS.num_multi_layers, FLAGS.multi_layer_size]
    ...
    X_train, X_val, X_test,         y_train, y_val, y_test,         cohorts_train, cohorts_val, cohorts_test = stratified_split(
